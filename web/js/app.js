...
(function () {
  ...
  function Btn({ onClick, children, variant = 'default', size = 'md', className, ...rest }) {
    ...
    return html`<button onClick=${onClick} class=${cx('inline-flex items-center justify-center gap-1.5 rounded-lg font-medium transition focus:outline-none focus:ring-2 focus:ring-blue-500/50 disabled:opacity-50', v, s, className)} ...${rest}>${children}</button>`;
  }

  function Card({ children, className }) { return html`<div class=${cx('rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900', className)}>${children}</div>`; }

  function KPI({ label, value, sub, tone, onClick }) {
    const tones = { red: 'text-red-600', amber: 'text-amber-600', green: 'text-emerald-600', blue: 'text-blue-600', slate: 'text-slate-700 dark:text-slate-200' };
    return html`<button onClick=${onClick} class=${cx('rounded-xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:shadow dark:border-slate-800 dark:bg-slate-900', onClick && 'hover:border-blue-400 cursor-pointer')}>
      <div class="text-xs font-medium uppercase tracking-wide text-slate-400">${label}</div>
      <div class=${cx('mt-1 text-2xl font-bold mono', tones[tone] || tones.slate)}>${value}</div>
  ...
  const inputCls = 'w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/30 dark:border-slate-700 dark:bg-slate-800';
  ...
  function FichaEquipo({ inv, onClose, openDrawer }) {
    ...
    return html`<${Drawer} open=true onClose=${onClose} title=${eq.inv} width="max-w-3xl">
      ...
      <div class="mb-4 flex gap-1 overflow-x-auto border-b border-slate-200 dark:border-slate-800">
        ${tabs.map(([k, l]) => html`<button key=${k} onClick=${() => setTab(k)} class=${cx('whitespace-nowrap px-3 py-2 text-sm font-medium border-b-2 -mb-px', tab === k ? 'border-blue-500 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700')}>${l}</button>`)}
      </div>
  ...
  function FichaPendientes({ eq, openDrawer }) {
    ...
    return html`<div class="space-y-2">${pends.map(p => html`<button key=${p.id} onClick=${() => openDrawer({ type: 'pendiente', pend: p })} class="block w-full rounded-lg border border-slate-200 p-3 text-left text-sm hover:border-blue-400 dark:border-slate-800">
  ...
  function ColaTrabajo({ go, openFicha, openDrawer }) {
    ...
    return html`<div class="space-y-6">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <${KPI} label="No operativos / ST" value=${caidos.length} tone="red" onClick=${() => go('correctivos')} sub="Pipeline correctivo" />
        <${KPI} label="Detenidos >30 días" value=${masTreinta.length} tone="red" onClick=${() => go('correctivos')} />
        <${KPI} label="Fuera de vida útil" value=${fueraVU.length} tone="amber" onClick=${() => go('equipos', { vur: true })} sub="vur < 0" />
        <${KPI} label="Pendientes vencidos" value=${pendVenc.length} tone="amber" onClick=${() => go('pendientes', { vencidos: true })} />
        <${KPI} label="Reprogramaciones" value=${reprogs.length} tone="blue" onClick=${() => go('pendientes', { tipo: 'reprogramacion' })} />
        <${KPI} label=${'MP ' + HHHA.MES_ESPANOL[HHHA.NUM_MES[mes]]} value=${pctMes + '%'} tone=${pctMes >= 80 ? 'green' : 'amber'} onClick=${() => go('mpmes')} sub=${ejecMes.length + '/' + progMes.length} />
      </div>
      ...
      <div class="grid gap-4 lg:grid-cols-2">
        <${Card}>
          ...
          ${caidos.length === 0 ? h(Empty, { title: 'Todo operativo 🎉' }) : html`<div class="space-y-1.5">
            ${caidos.slice().sort((a, b) => HHHA.diasEnEstado(b) - HHHA.diasEnEstado(a)).slice(0, 6).map(e => html`<button key=${e.inv} onClick=${() => openFicha(e.inv)} class="flex w-full items-center justify-between rounded-lg border border-slate-200 px-3 py-2 text-left text-sm hover:border-blue-400 dark:border-slate-800">
        ...
        <${Card}>
          ...
          ${pendVenc.length === 0 ? h(Empty, { title: 'Sin vencidos' }) : html`<div class="space-y-1.5">
            ${pendVenc.slice(0, 6).map(p => html`<button key=${p.id} onClick=${() => openDrawer({ type: 'pendiente', pend: p })} class="flex w-full items-center justify-between rounded-lg border border-slate-200 px-3 py-2 text-left text-sm hover:border-blue-400 dark:border-slate-800">
  ...
  function Correctivos({ openFicha, openDrawer }) {
    ...
    return html`<div class="space-y-4">
      ...
      ${cerrados.length > 0 && html`<${Card}>
        ...
        <div class="flex flex-wrap gap-2">${cerrados.map(c => { const eq = HHHA.findEquipo(c.inv) || {}; return html`<button key=${c.id} onClick=${() => openFicha(c.inv)} class="rounded-lg border border-slate-200 px-2.5 py-1 text-xs hover:border-blue-400 dark:border-slate-800"><span class="mono text-slate-400">${c.inv}</span> ${eq.equipo || ''} ✓</button>`; })}</div>
      </${Card}>`}
    </div>`;
  }
      ...
      ${modo === 'mes' ? html`<${Card}>
        ...
        <div class="flex items-end gap-2" style=${{ height: '160px' }}>
          ${trend.map(t => html`<div key=${t.m} class="flex flex-1 flex-col items-center justify-end gap-1">
            <div class="w-full rounded-t bg-blue-500/80" style=${{ height: Math.max(2, t.pct) + '%' }} title=${t.pct + '%'}></div>
  ...
  function App({ user }) {
    ...
    return html`<div class="flex h-full">
      ...
      <div class="flex min-w-0 flex-1 flex-col">
        <header class="sticky top-0 z-10 flex items-center gap-3 border-b border-slate-200 bg-white/90 px-4 py-2.5 backdrop-blur dark:border-slate-800 dark:bg-slate-900/90">
          <button class="rounded-lg p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 lg:hidden" onClick=${() => setSidebar(true)} aria-label="Menú">☰</button>
          <h1 class="text-base font-semibold">${titulo}</h1>
          <div class="ml-auto flex items-center gap-1.5">
            <button onClick=${() => setPalette(true)} class="hidden items-center gap-2 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs text-slate-400 hover:bg-slate-100 dark:border-slate-700 dark:hover:bg-slate-800 sm:flex">Buscar <kbd class="rounded bg-slate-100 px-1 dark:bg-slate-800">⌘K</kbd></button>
            <button onClick=${toggleDark} class="rounded-lg p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800" aria-label="Tema">${dark ? '☀' : '☾'}</button>
          </div>
        </header>
        <main class="scrollbar-thin flex-1 overflow-y-auto p-4 lg:p-6">${viewNode}</main>
      </div>
